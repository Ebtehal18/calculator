...
window.addEventListener("keydown", (e) => {
  // keyWithoutHover(e.key);
  keyWithHover(e.key);
});
const keyWithoutHover = (key) => {
  if (avaliableNumbers.includes(key)) {
    concatNumber(key);
  } else if (avlaiableOperations.includes(key)) {
    operationBtn(key);
  } else if (key === "Enter") {
    executeOperation();
